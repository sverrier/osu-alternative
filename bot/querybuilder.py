...
class QueryBuilder:
    ...
    def setFromClause(self):
        ...
        for i in range(1, len(table_order)):
            key = f"{table_order[i-1]},{table_order[i]}"
            if key in JoinClauses:
                self.fromclause += JoinClauses[key]
            else:
                raise ValueError(f"Missing join condition for {key}")

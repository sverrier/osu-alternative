CREATE OR REPLACE FUNCTION update_highest_score_taiko() RETURNS TRIGGER AS $$
BEGIN
    ...
    INSERT INTO scoreLive (
        ...
        statistics_ok, statistics_miss, statistics_large_bonus, statistics_ignore_hit,
        statistics_ignore_miss, statistics_small_bonus, total_score,
    ...
    SELECT
        ...
        NEW.maximum_statistics_large_bonus, NEW.maximum_statistics_small_bonus,
        NEW.maximum_statistics_ignore_hit, NEW.mods, NEW.passed, NEW.pp, NEW.preserve, NEW.processed,
        NEW.rank, NEW.ranked, NEW.replay, NEW.ruleset_id, NEW.started_at, NEW.statistics_great,
        NEW.statistics_ok, NEW.statistics_miss, NEW.statistics_large_bonus, NEW.statistics_ignore_hit,
        NEW.statistics_ignore_miss, NEW.statistics_small_bonus, NEW.total_score,
...
DROP TRIGGER IF EXISTS  set_highest_score_taiko on scoreTaiko;

-- ✅ Convert to AFTER trigger to avoid conflicts
CREATE TRIGGER set_highest_score_taiko
AFTER INSERT OR UPDATE ON scoreTaiko
FOR EACH ROW
WHEN (NEW.classic_total_score IS NOT NULL)
EXECUTE FUNCTION update_highest_score_taiko();